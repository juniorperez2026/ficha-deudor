...
export interface Documento {
  // 1. CAMPOS UNIVERSALES (Casi todos los clientes los tienen)
  // Usamos '?' en algunos porque no todos los clientes envían exactamente lo mismo
  id?: string;
  documento?: string;
  estado?: string;
  moneda?: string;
  ...
  // 2. FIRMA DE ÍNDICE (La clave de la escalabilidad)
  // Esto permite que el objeto tenga CUALQUIER otra propiedad 
  // (como 'placa', 'interes', 'tramo', 'cuenta_bt') sin marcar error.
  [key: string]: any; 
}
...
export interface DeudorHeaderInfo {
  grupo: string;
  cartera: string;
  cliente: string;
  deudor: string;
